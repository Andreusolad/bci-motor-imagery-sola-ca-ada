"""Build the final cross-experiment comparison once all runs are complete.

Reads ``metrics.json`` from every registered experiment directory
(``z_score/``, ``running_exponential/``, ``euclidean_alignment/``) and
writes ``comparison_metrics.csv``, ``comparison_metrics.json`` and
``comparison_report.md`` into the comparison directory, including an
automatic verdict on which normalization method generalizes best
cross-subject. Scales to any number of registered methods -- adding a new
one only requires it to have a ``metrics.json``.
"""
...
_DISPLAY_NAMES: Dict[str, str] = {
    "z_score": "EEGNet + Z-score",
    "running_exponential": "EEGNet + Running Exponential Standardization",
    "euclidean_alignment": "EEGNet + Euclidean Alignment",
}
...
# Metrics used for the "better cross-subject generalization" verdict, and
# their weights (trial-level metrics dominate, per the primary-metric
...
def build_comparison(window: str = "legacy") -> Dict[str, object]:
    """Load every experiment's metrics and write the comparison artifacts."""
...
def _write_report(path: Path, comparison_payload: Dict[str, object]) -> None:
    ...
    lines = [
        "# Comparison: Z-score vs. Running Exponential Standardization vs. Euclidean Alignment",
        "",
        f"Same EEGNet architecture, hyper-parameters, optimizer (AdamW + weight decay), "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build the EEGNet normalization comparison.")
