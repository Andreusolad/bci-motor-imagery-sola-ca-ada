"""CLI entry point: run one full EEGSym normalization-comparison experiment.
...
normalization step differs)::
    ...
    -> build EEGSym, train (AdamW + weight decay + label smoothing)
    -> evaluate (crop-level + trial-level via trial aggregation)
    -> write all required artifacts, then verify they all exist

Reuses (imports, never modifies) the generic, architecture-agnostic parts of
the EEGNet study -- data loading, cropping, normalization, leakage guard,
trial aggregation, evaluation and artifact writing -- because none of that
code is EEGNet-specific. Only the model (EEGSym), the training harness and
the config/comparability/output-directory wiring are local to this package.

Usage::

    .venv\\Scripts\\python.exe -m src.eegsym_study.run_experiment --method z_score
    .venv\\Scripts\\python.exe -m src.eegsym_study.run_experiment --method running_exponential
    .venv\\Scripts\\python.exe -m src.eegsym_study.run_experiment --method euclidean_alignment
"""
...
from ..study.crops import StudyCropArrays, build_crops
...
from .training import train_eegsym
...
def run_experiment(method: str, window: str = "legacy") -> Dict[str, object]:
    """Run one full EEGSym experiment end-to-end and return its metrics dict.
    ...
    try:
        logger.info("=== EEGSYM EXPERIMENT: %s (window=%s) ===", method, window)
        ...
        result = train_eegsym(crops_by_split["train"], crops_by_split["val"], output_dir)
        ...
        logger.info("=== EEGSYM EXPERIMENT %s (window=%s) COMPLETE: all artifacts verified in %s ===",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one EEGSym normalization-comparison experiment.")
