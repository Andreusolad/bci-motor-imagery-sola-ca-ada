"""Pre-flight scientific-comparability check for the EEGSym experiments.
...
the normalization method: EEGSym architecture, hyper-parameters, optimizer,
callbacks, seed, max epochs, split and evaluation procedure. If anything
else differs, a :class:`ComparabilityError` is raised and training must not
start.

Deliberately isolated from ``first_ml/src/study/comparability.py`` (same
logic, own module) so this package never has to import from or modify the
EEGNet study's files.
"""
...
_SHARED_KEYS = ("eegsym", "training", "dataset")
...
def build_experiment_config(method: str, normalization_params: Dict[str, object]) -> Dict[str, object]:
    ...
    return {
        ...
        "eegsym": study_config.eegsym_as_dict(),
...
def assert_comparable(*configs: Dict[str, object]) -> None:
    ...
    logger.info(
        "Comparability check passed: %d EEGSym experiments differ only in normalization "
